...
public class GameSystem {
    ...
    public static boolean addFoodToRefrigerator(int index, int amount) {
        if (index > market.getStock().size() || index < 0) {
            System.out.println("Error: Invalid index number!");
            return false;
        }
        int cost = market.getStock().get(index).getFood().getPrice() * amount;
        ...
        refrigerator.addFood(market.getStock().get(index).getFood(), amount);
